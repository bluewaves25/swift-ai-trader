        ...
        return (
          <div className="space-y-4 md:space-y-6 animate-fade-in">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 md:gap-4">
              <div>
                <h2 className="text-lg md:text-2xl lg:text-3xl font-bold tracking-tight bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                  Dashboard Overview
                </h2>
              ...
              <div className="flex gap-2">
                <Button 
                  size="sm" 
                  className="bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 text-xs md:text-sm h-8 md:h-10" 
                ...
                <Button 
                  size="sm" 
                  variant="destructive" 
                  className="shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 text-xs md:text-sm h-8 md:h-10"
              ...
              {[
                ...
                <Card key={stat.title} className="group relative overflow-hidden border-0 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900">
                  ...
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 md:pb-2 p-3 md:p-6">
                    <CardTitle className="text-xs md:text-sm font-medium text-gray-600 dark:text-gray-300">
                      {stat.title}
                    </CardTitle>
                    <stat.icon className={`h-3 w-3 md:h-4 md:w-4 text-${stat.color}-500`} />
                  </CardHeader>
              ...
              <CardContent className="p-3 md:p-6 pt-0">
                {systemHealth ? (
                  ...
                  <div className="flex items-center justify-center py-4 md:py-8">
                    <div className="animate-spin rounded-full h-4 w-4 md:h-8 md:w-8 border-b-2 border-blue-500"></div>
                    <span className="ml-2 md:ml-3 text-muted-foreground text-xs md:text-sm">Loading system health...</span>
                  </div>
                )}
              </CardContent>
            </Card>
            ...
            <div className="grid gap-4 md:gap-6 md:grid-cols-2">
              ...
              <Card className="border-0 shadow-lg">
                <CardHeader className="p-3 md:p-6">
                  <CardTitle className="text-sm md:text-lg">AUM Trend</CardTitle>
                  <CardDescription className="text-xs md:text-sm">Assets Under Management over time</CardDescription>
                </CardHeader>
                <CardContent className="p-3 md:p-6 pt-0">
              ...
              <Card className="border-0 shadow-lg">
                <CardHeader className="p-3 md:p-6">
                  <CardTitle className="text-sm md:text-lg">Top Strategies</CardTitle>
                  <CardDescription className="text-xs md:text-sm">Best performing strategies</CardDescription>
                </CardHeader>
                <CardContent className="p-3 md:p-6 pt-0">
                  <div className="space-y-2 md:space-y-4">
                    {topStrategies.length > 0 ? topStrategies.map((strategy, index) => (
                      <div key={strategy.id} className="flex items-center justify-between p-2 md:p-3 rounded-lg bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700 hover:from-blue-50 hover:to-purple-50 dark:hover:from-blue-900/20 dark:hover:to-purple-900/20 transition-all duration-300">
                        <div className="flex items-center space-x-2 md:space-x-3">
                          <div className={`w-6 h-6 md:w-10 md:h-10 rounded-full flex items-center justify-center text-xs md:text-sm font-bold ${
                          ...
                          <div>
                            <h4 className="font-semibold text-xs md:text-sm">{strategy.name}</h4>
                            <p className="text-xs text-muted-foreground">{strategy.description}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-green-600 text-xs md:text-sm">+{strategy.performance.toFixed(2)}%</p>
                          <p className="text-xs text-muted-foreground">{strategy.status}</p>
                        </div>
                      </div>
                    )) : (
                      <div className="text-center py-4 md:py-8 text-muted-foreground">
                        <TrendingUp className="h-8 w-8 md:h-12 md:w-12 mx-auto mb-2 md:mb-3 opacity-50" />
                        <p className="text-xs md:text-sm">No strategy data available</p>
                      </div>
                    )}
                  </div>
                </CardContent>
            ...
            <Card className="border-0 shadow-lg">
              <CardHeader className="p-3 md:p-6">
                ...
                <CardDescription className="text-xs md:text-sm">Latest system events and logs</CardDescription>
              </CardHeader>
              <CardContent className="p-3 md:p-6 pt-0">
                <div className="space-y-2 md:space-y-3 max-h-48 md:max-h-64 overflow-y-auto">
                  {logs.length > 0 ? logs.map((log, index) => (
                    <div key={index} className="flex items-start space-x-2 md:space-x-3 p-2 md:p-3 rounded-lg bg-gray-50 dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors duration-300">
                      <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-blue-500 mt-1 md:mt-2 flex-shrink-0"></div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs md:text-sm font-medium text-gray-900 dark:text-white">{log.event}</p>
                        <p className="text-xs text-muted-foreground font-mono">{log.timestamp}</p>
                      </div>
                    </div>
                  )) : (
                    <div className="text-center py-4 md:py-8 text-muted-foreground">
                      <Activity className="h-8 w-8 md:h-12 md:w-12 mx-auto mb-2 md:mb-3 opacity-50" />
      ...
      <SidebarProvider>
        <div className="min-h-screen flex w-full bg-background">
          ...
          <div className="flex-1 flex flex-col min-w-0 max-w-full">
            {/* Fixed Header */}
            <header className="sticky top-0 z-40 border-b bg-background/80 backdrop-blur-md supports-[backdrop-filter]:bg-background/80 p-2 md:p-4 shadow-sm">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-1 md:space-x-4 flex-shrink-0">
                  <Badge variant="outline" className="px-1 md:px-2 py-1 text-xs border-green-200 text-green-700 bg-green-50 dark:border-green-800 dark:text-green-400 dark:bg-green-900/20">
